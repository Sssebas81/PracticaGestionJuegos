...
export class Participant {
    ...
    @ManyToOne(() => Session, (session) => session.participants, { nullable: false, eager:true}) // Many-to-one relationship with Session entity, meaning that each participant is associated with one session, but a session can have many participants
    ...
    @ManyToOne(() => User, (user) => user.participations, { nullable: false, eager: true }) // Many-to-one relationship with User entity, meaning that each participant is associated with one user, but a user can be a participant in many sessions
